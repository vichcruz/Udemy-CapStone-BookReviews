...
app.get("/order", async (req, res) => {
  try {
    ...
    const response = await db.query(`select * from books order by ${ordering}`);
...
// get route to return search results
...
app.get("/book-detail/:id", async (req, res) => {
  try {
    const response = await db.query("select * from books where id = $1", [
      req.params.id,
    ]);
    ...
    const chapters = await db.query(
      "select * from chapters where book_id = $1 order by position_in_book",
      [req.params.id]
    );
    console.log(chapters.rows);

    res.status(200).render("bookDetailView.ejs", {
      showAddBookButton: false,
      showOrderButton: false,
      showSearchBar: false,
...
app.get("/add-note/:bookId", async (req, res) => {
  try {
    const result = await db.query(
      `select * from chapters where book_id = ${req.params.bookId} order by position_in_book`
    );
...
app.post("/books", async (req, res) => {
  try {
    ...
    const newBook = await db.query(
      "insert into books (title, review, rating, date_read, author_lastname, isbn) values ($1, $2, $3, $4, $5, $6) returning id;",
      [title, review, rating, dateRead, authorLastName, isbn]
    );

    let position = 1;
    for (const chapter of req.body.chapters) {
      await db.query(
        "insert into chapters (position_in_book, title, book_id) values ($1, $2, $3);",
        [position, chapter, newBook.rows[0].id]
      );
      position++;
    }
...
app.put("/books/:id", async (req, res) => {
  try {
    const oldReponse = await db.query("select * from books where id = $1", [
      req.params.id,
    ]);
    ...
    await db.query(
      "update books set title = $1, review = $2, rating = $3, date_read = $4, author_lastname = $5 where id = $6",
      [
...
app.post("/notes/:bookId", async (req, res) => {
  try {
    ...
    await db.query(
      "insert into notes (text, book_id, chapter_id, created_at) values ($1, $2, $3, $4)",
      [text, req.params.bookId, chapterId, new Date()]
    );
...
app.patch("/notes/:noteId", async (req, res) => {
  try {
    const oldResponse = await db.query("select * from notes where id = $1", [
      req.params.noteId,
    ]);
    ...
    await db.query("update notes set text = $1 where id = $2", [
      text,
      req.params.noteId,
    ]);
...
app.delete("/notes/:noteId", async (req, res) => {
  try {
    ...
    res.status(500).send("An error occurred while deleting note:", error);
  }
});
...
app.delete("/chapters/:chapterId", async (req, res) => {
  try {
    const response = await db.query("select * from chapters where id = $1", [
    ...
    await db.query("delete from chapters where id = $1", [
      req.params.chapterId,
    ]);
